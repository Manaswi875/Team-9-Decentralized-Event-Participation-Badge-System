...
interface IParticipationBadge {
     ...
     *
     * Reverts if the address has already received a badge for this event,
     * preventing duplicate mints for the same wallet + event combination.
     *
    ...
    /**
     * Checks whether a specific wallet already holds a badge for a given event.
     *
     * Useful for the backend relayer and any external verifier to quickly check
     * eligibility before attempting a mint, without needing to scan all tokens.
     */
    function hasBadge(address wallet, string memory eventId) external view returns (bool);
}
